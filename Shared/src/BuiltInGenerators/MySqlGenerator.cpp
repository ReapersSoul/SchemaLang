...
std::string MysqlGenerator::generate_create_table_statement_string_struct(std::shared_ptr<ProgramStructure>ps, StructDefinition &s)
{
	...
	for (int i = 0; i < s.getMemberVariables().size(); i++)
	{
		...
		sql += "  " + escape_identifier(s.getMemberVariables()[i].identifier) + " " + convert_to_local_type(ps, s.getMemberVariables()[i].type);
		...
		if (s.getMemberVariables()[i].auto_increment)
		{
			sql += " AUTO_INCREMENT";
		}
		...
		if (!s.getMemberVariables()[i].default_value.empty())
		{
			sql += " DEFAULT '" + s.getMemberVariables()[i].default_value + "'";
		}
		
		// Collect foreign key constraints to add later
		if (!s.getMemberVariables()[i].reference.variable_name.empty())
		{
			std::string reference_constraint = "  FOREIGN KEY (" + escape_identifier(s.getMemberVariables()[i].identifier) + ") REFERENCES " + 
				escape_identifier(s.getMemberVariables()[i].reference.struct_name) + "(" + escape_identifier(s.getMemberVariables()[i].reference.variable_name) + ")";
...
std::string MysqlGenerator::generate_select_by_member_variable_statement_string(StructDefinition &s, MemberVariableDefinition &mv_1, std::vector<int> &criteria)
{
	...
	{
		sql += escape_identifier(s.getMemberVariables()[criteria[i]].identifier) + " = ?";
...
std::string MysqlGenerator::generate_insert_statement_string_struct(StructDefinition &s)
{
	...
	for (int i = 0; i < s.getMemberVariables().size(); i++)
	{
		...
		sql += escape_identifier(s.getMemberVariables()[i].identifier);
		first = false;
	}
	sql += ") VALUES (";
	first = true;
...
std::string MysqlGenerator::generate_update_all_statement_string_struct(StructDefinition &s)
{
	...
	if (!has_primary_key)
	{
		...
		sql += escape_identifier(s.getMemberVariables()[0].identifier) + " = ?";
	}
	sql += ";";
	return sql;
}
...
void MysqlGenerator::generate_select_member_variable_function_statement(std::shared_ptr<Generator>gen, std::shared_ptr<ProgramStructure>ps, StructDefinition &s, MemberVariableDefinition &mv_1, std::vector<int> &criteria)
{
	...
	select_statement.generate_function = [this, &mv_1, &criteria](std::shared_ptr<Generator>gen, std::shared_ptr<ProgramStructure>ps, StructDefinition &s, FunctionDefinition &fd, std::ostream &structFile)
	{
		...
		for (int i = 0; i < criteria.size(); i++)
		{
			...
			structFile << "\t\twhere_clause += \"" << escape_identifier(s.getMemberVariables()[criteria[i]].identifier) << " = :param" << i << "\";\n";
		}
		...
		for (int i = 0; i < criteria.size(); i++)
		{
			structFile << "\t\t\t.bind(\"param" << i << "\", " << s.getMemberVariables()[criteria[i]].identifier << ")\n";
		}
...
void MysqlGenerator::generate_insert_statements_function_struct(std::shared_ptr<Generator>gen, std::shared_ptr<ProgramStructure>ps, StructDefinition &s)
{
	...
	insert_statement.generate_function = [this](std::shared_ptr<Generator>gen, std::shared_ptr<ProgramStructure>ps, StructDefinition &s, FunctionDefinition &fd, std::ostream &structFile)
	{
		...
		for (int i = 0; i < s.getMemberVariables().size(); i++)
		{
			...
			structFile << "\"" << escape_identifier(s.getMemberVariables()[i].identifier) << "\"";
			first = false;
		}
		structFile << ")\n";
		structFile << "\t\t\t.values(";
		...
		for (int i = 0; i < s.getMemberVariables().size(); i++)
		{
			...
			if (s.getMemberVariables()[i].type.is_required())
			{
				...
				if (s.getMemberVariables()[i].type.is_enum(ps)) {
					structFile << "(int)" << s.getMemberVariables()[i].identifier;
				} else {
					structFile << s.getMemberVariables()[i].identifier;
				}
			}
			else
			{
				...
				if (s.getMemberVariables()[i].type.is_enum(ps)) {
					structFile << "(" << s.getMemberVariables()[i].identifier << ".has_value() ? mysqlx::Value((int)" << s.getMemberVariables()[i].identifier << ".value()) : mysqlx::nullvalue)";
				} else {
					structFile << "(" << s.getMemberVariables()[i].identifier << ".has_value() ? mysqlx::Value(" << s.getMemberVariables()[i].identifier << ".value()) : mysqlx::nullvalue)";
				}
			}
			first = false;
		}
	...
	insert_statement_no_args.generate_function = [this](std::shared_ptr<Generator>gen, std::shared_ptr<ProgramStructure>ps, StructDefinition &s, FunctionDefinition &fd, std::ostream &structFile)
	{
		...
		for (int i = 0; i < s.getMemberVariables().size(); i++)
		{
			...
			if (s.getMemberVariables()[i].type.is_required())
			{
				...
				if (s.getMemberVariables()[i].type.is_enum(ps)) {
					structFile << "(int)this->" << s.getMemberVariables()[i].identifier;
				} else {
					structFile << "this->" << s.getMemberVariables()[i].identifier;
				}
			}
			else
			{
				...
				if (s.getMemberVariables()[i].type.is_enum(ps)) {
					structFile << "(this->" << s.getMemberVariables()[i].identifier << ".has_value() ? mysqlx::Value((int)this->" << s.getMemberVariables()[i].identifier << ".value()) : mysqlx::nullvalue)";
				} else {
					structFile << "(this->" << s.getMemberVariables()[i].identifier << ".has_value() ? mysqlx::Value(this->" << s.getMemberVariables()[i].identifier << ".value()) : mysqlx::nullvalue)";
				}
			}
			first = false;
		}
...
void MysqlGenerator::generate_update_all_statement_function_struct(std::shared_ptr<Generator>gen, std::shared_ptr<ProgramStructure>ps, StructDefinition &s)
{
	...
	update_all_statement.generate_function = [this](std::shared_ptr<Generator>gen, std::shared_ptr<ProgramStructure>ps, StructDefinition &s, FunctionDefinition &fd, std::ostream &structFile)
	{
		...
		{
			structFile << "\t\tupdate.where(\"" << escape_identifier(s.getMemberVariables()[0].identifier) << " = :pk\").bind(\"pk\", " << s.getMemberVariables()[0].identifier << ");\n";
		}
...
void MysqlGenerator::generate_delete_statement_function_struct(std::shared_ptr<Generator>gen, std::shared_ptr<ProgramStructure>ps, StructDefinition &s)
{
	...
	delete_statement.generate_function = [this](std::shared_ptr<Generator>gen, std::shared_ptr<ProgramStructure>ps, StructDefinition &s, FunctionDefinition &fd, std::ostream &structFile)
	{
		...
		if (!s.getMemberVariables().empty())
		{
			...
			{
				structFile << "\t\tmysqlx::Result result = table.remove()\n";
				structFile << "\t\t\t.where(\"" << escape_identifier(s.getMemberVariables()[0].identifier) << " = :param\")\n";
				structFile << "\t\t\t.bind(\"param\", " << s.getMemberVariables()[0].identifier << ")\n";
				structFile << "\t\t\t.execute();\n";
			}
		}
	...
	delete_statement_no_args.generate_function = [this](std::shared_ptr<Generator>gen, std::shared_ptr<ProgramStructure>ps, StructDefinition &s, FunctionDefinition &fd, std::ostream &structFile)
	{
		...
		if (!s.getMemberVariables().empty())
		{
			...
			if (!has_primary_key)
			{
				structFile << "\t\tmysqlx::Result result = table.remove()\n";
				structFile << "\t\t\t.where(\"" << escape_identifier(s.getMemberVariables()[0].identifier) << " = :param\")\n";
				structFile << "\t\t\t.bind(\"param\", this->" << s.getMemberVariables()[0].identifier << ")\n";
				structFile << "\t\t\t.execute();\n";
			}
		}
...
bool MysqlGenerator::generate_select_files(std::shared_ptr<ProgramStructure>ps, StructDefinition &s, std::string out_path)
{
	...
	for (int i = 0; i < s.getMemberVariables().size(); i++)
	{
		...
		{
			std::string filename = out_path + "/" + s.getIdentifier() + "_select_" + s.getMemberVariables()[i].identifier + "_by";
			std::string sql = generate_select_by_member_variable_statement_string(s, s.getMemberVariables()[i], combinations[j]);
			for (int k = 0; k < combinations[j].size(); k++)
			{
				filename += "_" + s.getMemberVariables()[combinations[j][k]].identifier;
			}
			filename += ".sql";
...
bool MysqlGenerator::add_generator_specific_content_to_struct(std::shared_ptr<Generator>gen, std::shared_ptr<ProgramStructure>ps, StructDefinition &s)
{
	...
	// 	s.add_include("<mysqlx/xdevapi.h>");
	// 	s.add_include("<string>");
	// 	s.add_include("<vector>");
	// 	s.add_include("<iostream>");
	...
	// 				return true;
	// 			};
	// 			s.add_private_variable(primary_key_index);
	// 			has_primary_key = true;
	// 		}
...
bool MysqlGenerator::generate_files(std::shared_ptr<ProgramStructure> ps, std::string out_path)
{
	...
	try
	{
		std::string version_table_content(
			reinterpret_cast<const char *>(loadSchemaLangShared_ResourcesEmbeddedFile("/MySQL/_schema_versions_create_table.sql").data()),
			loadSchemaLangShared_ResourcesEmbeddedFile("/MySQL/_schema_versions_create_table.sql").size()
		);
...
bool MysqlGenerator::generate_migration_files(std::shared_ptr<ProgramStructure> ps, std::string out_path)
{
	if (!std::filesystem::exists(out_path))
	{
		std::filesystem::create_directories(out_path);
	...
	auto& migs = ps->getMigrations();
	if (migs.empty())
	{
		PLOGI << "No migrations to generate for MySQL" << std::endl;
		return true;
	}

	// Group migrations by struct
	std::map<std::string, std::vector<MigrationDefinition>> migrationsByStruct;
	for (const auto& migration : migs)
	{
		migrationsByStruct[migration.structName].push_back(migration);
	}

	// Generate migration SQL files
	for (const auto& [structName, migrations] : migrationsByStruct)
	{
		for (const auto& migration : migrations)
		{
			std::string filename = structName + "_" + 
								   std::to_string(migration.fromVersion.major) + "_" + 
								   std::to_string(migration.fromVersion.minor) + "_" + 
								   std::to_string(migration.fromVersion.patch) + "_to_" + 
								   std::to_string(migration.toVersion.major) + "_" + 
								   std::to_string(migration.toVersion.minor) + "_" + 
								   std::to_string(migration.toVersion.patch) + ".sql";

			std::ofstream migrationFile(out_path + "/" + filename);
			if (!migrationFile.is_open())
			{
				PLOGE << "Failed to create migration file: " << filename << std::endl;
				return false;
			}

			// Write migration header
			migrationFile << "-- Migration: " << structName << " from " 
						  << migration.fromVersion.major << "." << migration.fromVersion.minor << "." << migration.fromVersion.patch
						  << " to " 
						  << migration.toVersion.major << "." << migration.toVersion.minor << "." << migration.toVersion.patch << "\\n";
			migrationFile << "-- Generated by SchemaLang Transpiler\\n\\n";

			// Begin transaction
			migrationFile << "START TRANSACTION;\\n\\n";

			// Generate ALTER TABLE statements for each operation
			for (const auto& op : migration.operations)
			{
				switch (op.type)
				{
					case MigrationOperation::Type::AddField:
					{
						try
						{
							StructDefinition& currentStruct = ps->getStruct(structName);
							for (const auto& mv : currentStruct.getMemberVariables())
							{
								if (mv.identifier == op.fieldName)
								{
									std::string sqlType = convert_to_local_type(ps, mv.type);
									migrationFile << "ALTER TABLE `" << structName << "` ADD COLUMN `" 
												  << op.fieldName << "` " << sqlType;
									
									if (mv.type.is_required())
									{
										migrationFile << " NOT NULL DEFAULT ";
										if (mv.type.is_integer()) migrationFile << "0";
										else if (mv.type.is_real()) migrationFile << "0.0";
										else if (mv.type.is_bool()) migrationFile << "FALSE";
										else if (mv.type.is_string() || mv.type.is_char()) migrationFile << "''";
										else migrationFile << "NULL";
									}
									migrationFile << ";\\n";
									break;
								}
						...
						{
							PLOGE << "Error generating AddField migration: " << e.what() << std::endl;
							migrationFile << "-- Error: Could not add field " << op.fieldName << "\\n";
						}
						break;
					}
					case MigrationOperation::Type::RemoveField:
					{
						migrationFile << "-- WARNING: Dropping column " << op.fieldName << " will cause data loss\\n";
						migrationFile << "ALTER TABLE `" << structName << "` DROP COLUMN `" << op.fieldName << "`;\\n";
						break;
					}
					case MigrationOperation::Type::RenameField:
					{
						if (op.newFieldName.has_value())
						{
							...
							try
							{
								...
								for (const auto& mv : currentStruct.getMemberVariables())
								{
									if (mv.identifier == op.newFieldName.value())
									{
										std::string sqlType = convert_to_local_type(ps, mv.type);
										migrationFile << "ALTER TABLE `" << structName << "` CHANGE COLUMN `" 
													  << op.fieldName << "` `" << op.newFieldName.value() << "` " << sqlType;
										if (mv.type.is_required())
										{
											migrationFile << " NOT NULL";
										}
										migrationFile << ";\\n";
										break;
									}
							...
							{
								// Fallback if we can't find the new field
								migrationFile << "-- ALTER TABLE `" << structName << "` RENAME COLUMN `" 
											  << op.fieldName << "` TO `" << op.newFieldName.value() << "`;\\n";
								migrationFile << "-- Note: MySQL RENAME COLUMN requires MySQL 8.0+. Use CHANGE COLUMN for older versions.\\n";
							}
						}
						break;
					}
					case MigrationOperation::Type::ChangeType:
					{
						...
						{
							migrationFile << "-- WARNING: Changing column type may cause data loss or conversion errors\\n";
							migrationFile << "ALTER TABLE `" << structName << "` MODIFY COLUMN `" 
										  << op.fieldName << "` " << op.newType.value() << ";\\n";
						}
						break;
					}
					default:
						migrationFile << "-- Unsupported migration operation\\n";
						break;
				}
			}
			...
			migrationFile << "INSERT INTO _schema_versions (table_name, version_major, version_minor, version_patch, applied_at, migration_file) VALUES ("
						  << "'" << structName << "', "
						  << migration.toVersion.major << ", "
						  << migration.toVersion.minor << ", "
						  << migration.toVersion.patch << ", "
						  << "NOW(), "
						  << "'" << filename << "'"
						  << ") ON DUPLICATE KEY UPDATE "
						  << "version_major = " << migration.toVersion.major << ", "
						  << "version_minor = " << migration.toVersion.minor << ", "
						  << "version_patch = " << migration.toVersion.patch << ", "
						  << "applied_at = NOW(), "
						  << "migration_file = '" << filename << "';\\n\\n";

			// Commit transaction
			migrationFile << "COMMIT;\\n";

			migrationFile.close();
			PLOGI << "Generated migration file: " << filename << std::endl;
		}
	}
