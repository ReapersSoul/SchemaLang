...
{% if not field.type.is_array %}{% set current_field = current_field + 1 %}
    {{ field.identifier }} {{ SQLite_convert_to_local_type(field.type) }} {% if field.required %} NOT NULL {% endif %}{% if field.unique %} UNIQUE {% endif %}{% if field.primary_key %} PRIMARY KEY {% endif %}{% if field.auto_increment %} AUTOINCREMENT {% endif %}{% if not field.reference.struct_name=="" %} REFERENCES{{field.reference.struct_name}}({{field.reference.variable_name}}) {% endif %}{% if field.default_value %} DEFAULT {{SQLite_format_default(field.type, field.default_value)}}{% endif %}{% if current_field < field_count or additional_field_count > 0 %},{% endif %}
